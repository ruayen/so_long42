...
void	get_textures(t_game *game)
{
	...
	game->textu->player = mlx_load_png("./images/luffy1.png");
	game->textu->coin = mlx_load_png("./images/m.png");
	...
	game->textu->exit2 = mlx_load_png("./images/exit.png");
}
...
void	get_images(t_game *game)
{
	...
	game->imag->exit2 = mlx_texture_to_image(game->mlx, game->textu->exit2);
...
void	draw_floor(t_game *game, t_images *image)
{
	...
	while (game->map[y])
	{
		...
		while (game -> map[y][x])
		{
			...
			{
				mlx_image_to_window(game->mlx, image->exit2, x * 64, y * 64);
	...
	while (game->map[y])
	{
		...
		while (game -> map[y][x])
		{
			if (game->map[y][x] == MAP_COLLECTOR)
				mlx_image_to_window(game->mlx, image->coin, x * 64, y * 64);
			if (game->map[y][x] == MAP_START)
				mlx_image_to_window(game->mlx, image->player, x * 64, y * 64);
			x++;
		}
		y++;
	}
